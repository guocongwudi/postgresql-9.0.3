...
static pid_t StartupPID = 0, BgWriterPID = 0, WalWriterPID = 0, WalReceiverPID =
		0, AutoVacPID = 0, PgArchPID = 0, PgStatPID = 0, SysLoggerPID = 0;
...
int PostmasterMain(int argc, char *argv[]) {
	...
	while ((opt = getopt(argc, argv,
		...
		switch (opt) {
		case 'A':
			SetConfigOption("debug_assertions", optarg, PGC_POSTMASTER,
					PGC_S_ARGV);
			break;

		case 'B':
			if (isdigit(optarg))
				fprintf(stderr, "get ############argv    \n############## %s", optarg);
		...
		case 'f':
			if (!set_plan_disabling_options(optarg, PGC_POSTMASTER,
					PGC_S_ARGV)) {
		...
		case 'h':
			SetConfigOption("listen_addresses", optarg, PGC_POSTMASTER,
					PGC_S_ARGV);
			break;
		...
		case 'k':
			SetConfigOption("unix_socket_directory", optarg, PGC_POSTMASTER,
					PGC_S_ARGV);
			break;
		...
		case 'N':
			SetConfigOption("max_connections", optarg, PGC_POSTMASTER,
					PGC_S_ARGV);
			break;
		...
		case 'W':
			SetConfigOption("post_auth_delay", optarg, PGC_POSTMASTER,
					PGC_S_ARGV);
			break;
	...
	/* For debugging: display postmaster environment */
	{
		...
		ereport(
				DEBUG3,
				(errmsg_internal("%s: PostmasterMain: initial environ dump:", progname)));
		ereport(DEBUG3,
				(errmsg_internal("-----------------------------------------")));
		for (p = environ; *p; ++p)
	...
	if (status != STATUS_OK
		...
		ereport(WARNING, (errmsg("could not create Unix-domain socket")));
#endif
...
static void pmdaemonize(void) {
	...
	if (pmlog < 0) {
		write_stderr("%s: could not open log file \"%s/%s\": %s\n", progname,
				DataDir, pmlogname, strerror(errno));
		ExitPostmaster(1);
	}
...
static int ServerLoop(void) {
	...
	for (;;) {
		...
		if (AutoVacPID == 0 && (AutoVacuumingActive() || start_autovac_launcher)
				&& pmState == PM_RUN) {
		...
		while (offset < len) {
			...
			if (strcmp(nameptr, "database") == 0)
				...
				if (!parse_bool(valptr, &am_walsender))
					ereport(
							FATAL,
							(errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("invalid value for boolean option \"replication\"")));
			} else {
	...
	if (WIFEXITED(exitstatus))
		...
		ereport(
				...
				/*------
				 translator: %s is a noun phrase describing a child process, such as
				 "server process" */
				(errmsg("%s (PID %d) was terminated by signal %d: %s", procname, pid, WTERMSIG(exitstatus), WTERMSIG(exitstatus) < NSIG ? sys_siglist[WTERMSIG(exitstatus)] : "(unknown)")));
#else
		ereport(lev,
...
static void PostmasterStateMachine(void) {
	...
	if (pmState == PM_WAIT_BACKENDS) {
		...
		if (CountChildren(BACKEND_TYPE_NORMAL | BACKEND_TYPE_AUTOVAC) == 0
				...
				&& (BgWriterPID == 0 || !FatalError) && WalWriterPID == 0
				&& AutoVacPID == 0) {
			if (FatalError) {
				/*
	...
	if (pmState == PM_WAIT_DEAD_END) {
		 ...
		 */
		if (DLGetHead(BackendList) == NULL && PgArchPID == 0
				&& PgStatPID == 0) {
			/* These other guys should be dead already */
			Assert(StartupPID == 0);Assert(WalReceiverPID == 0);Assert(BgWriterPID == 0);Assert(WalWriterPID == 0);Assert(AutoVacPID == 0);
...
static void sigusr1_handler(SIGNAL_ARGS) {
	...
	if (CheckPostmasterSignal(PMSIGNAL_START_WALRECEIVER) && WalReceiverPID == 0
			&& (pmState == PM_STARTUP || pmState == PM_RECOVERY
